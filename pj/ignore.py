...
def detai():
    try:
        ...
        cur.execute("SELECT iddt,tendetai,urldetai FROM detai")
        row = cur.fetchall()
        for i in row:
            key = i.get('iddt')
            values = i.get('tendetai')
            values1 = i.get('urldetai')
            dict_result[key] = values, values1
        return make_response(jsonify({"document": dict_result}), 200)
...
def updateRecommend(iddt, arrRecommend):
    ...
    for x in arrRecommend:
        cur.execute(
            "INSERT INTO goiy(iddt,MaSachGoiY,DoTuongTu) VALUES (%s, %s, %s)", (iddt, x[0], x[1]))
    cur.close()
    conn.commit()
    conn.close()
        ...
        for x in range(len(d)):
            ...
            for y in range(len(cosine[0])):
                ...
                if cosine[0][y] > 0:
                    ...
                    print("Index Book In Array "+str(y) +
